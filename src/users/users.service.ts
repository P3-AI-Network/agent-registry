...
export class UsersService {
    ...
    async createUser(createUserDto: CreateUserDto): Promise<User> {
        ...
        try {
            ...
            const signersAddress = ethers.verifyMessage(createUserDto.message, createUserDto.signature);
            console.log(signersAddress, createUserDto.walletAddress);
            if (signersAddress !== createUserDto.walletAddress) {
            ...
            if (response.status !== 201) {
                throw new BadRequestException(`Failed to create DID, Error: ${responseJsn.message}`);
            }
